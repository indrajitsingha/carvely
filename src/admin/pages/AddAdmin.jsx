...
const AddAdmin = () => {
  ...
  useEffect(() => {
    AllAdminData().then((Admin) => setAdminData(Admin.docs));

    console.log("I am AddAdmin")
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  var onFormSubmit = async () => {

    if (addAdmin.Password === addAdmin.CPassword) {

      await createUserWithEmailAndPassword(auth, addAdmin.Email, addAdmin.Password)
        .then((err) => setError(err))
      await RegisterNewAdmin(addAdmin)
      dispatch(logoutAuth())
    }
    else if (addAdmin.Password !== addAdmin.CPassword) {
      setError("Please Check Email And Password")
    }
    else {
      setError("Please Check Email Already Exist")
    }
  }
  ...
  const RegisterNewAdmin = async (addAdmin) => {

    await addDoc(collection(fireStoreDb, 'AdminData'), {
      id: uuidv4(),
      Name: addAdmin.DisplayName,
      Email: addAdmin.Email.toLowerCase(),
      Perks: addAdmin.Perks,
      SuperAdmin: user,
    })
  }
  ...
  return (
    ...
    <>
      ...
      {
        isModelOpen === true ?
          <div className="w-[100%] min-h-screen bg-gray-900 flex items-center justify-center px-5  mb-4 backdrop-blur-sm bg-white/90 flex-col">
            <span className='altButton bg-red-500  px-4 text-white rounded-full my-2 cursor-pointer'>After Adding Somebody you have to re-log in again in your account.</span>
                  ...
                  <div>
                    <div className="flex -mx-3">
                      <div className="w-1/2 px-3 mb-2 ">
                        <label className="text-xs font-semibold px-1">Display name</label>
                        <div className="flex">
                          <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i className="mdi mdi-account-outline text-gray-400 text-lg"></i></div>
                          <input type="text" name="DisplayName" value={addAdmin.DisplayName} onChange={handelChange} className="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Jhon Cena " required />
                        </div>
                      </div>
                      <div className="w-1/2 px-3 mb-2">
                        <label className="text-xs font-semibold px-1">Roll Perks</label>
                        <div className="flex">
                          <select name="Perks" className='block w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring' value={addAdmin.Perks} onChange={handelChange}>
                            <option value="Admin" defaultValue >Admin</option>
                            <option value="SuperAdmin" className=" bg-blue-900 hover:bg-blue-900" >SuperAdmin</option>
                          </select>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex -mx-3">
                      <div className="w-full px-3 mb-2">
                        ...
                        <div className="flex">
                          <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i className="mdi mdi-account-outline text-gray-400 text-lg"></i></div>
                          <input type="Email" name="Email" value={addAdmin.Email} onChange={handelChange} className="w-full -ml-10 pl-4 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="example@gmail.com" required />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex -mx-3">
                      <div className="w-full px-3 mb-2">
                        ...
                        <div className="flex">
                          ...
                          <input type="password" name="Password" value={addAdmin.Password} onChange={handelChange} className="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="****8 digit password****" required />
                        </div>
                      </div>
                    </div>
                    <div className="flex -mx-3">
                      <div className="w-full px-3 mb-12">
                        <label className="text-xs font-semibold px-1">Conform Password</label>
                        <div className="flex">
                          <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i className="mdi mdi-lock-outline text-gray-400 text-lg"></i></div>
                          <input type="password" name="CPassword" value={addAdmin.CPassword} onChange={handelChange} className="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="****8 digit Conform passwod *****" required />
                        </div>
                      </div>
                    </div>
                    <div className="flex -mx-3">
                      <div className="w-1/2 px-3 mb-2">
                        <button className="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded px-3 py-3 font-semibold" onClick={onFormSubmit}>REGISTER NOW</button>
                      </div>
                      <div className="w-1/2 px-3 mb-2">
                        <button className="block w-full max-w-xs mx-auto bg-red-300 hover:bg-red-500  text-white rounded px-3 py-3 font-semibold" onClick={() => setModel(false)}>Cancel</button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          :
          <div className="flex flex-col min-h-screen w-[100%] overflow-hidden">
            <h2 className="font-semibold text-gray-800 text-2xl text-center w-[100%] mt-[5vh]">Admin Users</h2>

            <div className="flex justify-center items-center h-[20%] w-[100%]  mt-4">
              <button className=" bg-[crimson] h-[50px] w-[320px] rounded text-white" onClick={() => setModel(true)}>

                Add Admin
              </button>
            </div>

            <div className="h-auto w-[100%] flex justify-center  items-center mb-10 mt-4  ">

              <div className="flex justify-center flex-col items-cente h-[100%] w-[100%] lg:w-[70%]">
                  ...
                  <table className="table-auto w-[100%]">
                    <thead className="text-xs font-semibold uppercase ">
                      <tr>
                        ...
                        <th className="p-2 whitespace-nowrap">
                          <div className="font-semibold text-left">Perks</div>
                        </th>
                        <th className="p-2 whitespace-nowrap">
                    ...
                    <tbody className="text-sm divide-y divide-gray-100">
                      ...
                      {
                        AdminData && AdminData.map((x, index) => {
                          return (

                            <tr className=" h-[5vh]" key={index}>
                              <td className="p-2 whitespace-nowrap">
                                <div className="flex items-center">
                                  <div className="font-medium text-gray-800">{index + 1}</div>
                                  <div className="w-10 h-10 flex-shrink-0 sm:mr-3 hidden lg:block">
                                    <img
                              ...
                              <td className="p-2 whitespace-nowrap">
                                <div className="text-left">{x.data().Name.slice(0, 7)}</div>
                              </td>
                              <td className="p-2 whitespace-nowrap">
                                <div className="text-left font-medium text-green-500">
                                  {x.data().Email.slice(0, 15)}
                                </div>
                              </td>
                              <td className="p-2 whitespace-nowrap">
                                <div className="text-left font-medium text-green-500">
